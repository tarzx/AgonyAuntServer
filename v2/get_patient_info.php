...
if (isset($_GET["pid"])) {
    ...
    $result = mysql_query("SELECT c1.pid, c1.name, (YEAR(CURDATE())-c1.birth_year) AS age, c1.gender, IF(c1.set_frequency=0, NULL, c2.frequency1) AS frequency1, IF(c1.set_frequency=0, NULL, c2.frequency2) AS frequency2, IF(c1.set_frequency=0, NULL, c2.frequency3) AS frequency3, IF(c1.set_frequency=0, NULL, c2.frequency4) AS frequency4, IF(c1.set_frequency=0, NULL, c2.frequency5) AS frequency5, IF(c1.set_frequency=0, NULL, c2.frequency6) AS frequency6, IF(c1.set_frequency=0, NULL, c2.frequency7) AS frequency7, IF(c1.set_slot=0, NULL, c2.slot1) AS slot1, IF(c1.set_slot=0, NULL, c2.slot2) AS slot2, IF(c1.set_slot=0, NULL, c2.slot3) AS slot3, IF(c1.set_slot=0, NULL, c2.slot4) AS slot4, IF(c1.set_slot=0, NULL, c2.slot5) AS slot5, IF(c1.set_slot=0, NULL, c2.slot6) AS slot6 FROM CBT_patients c1 JOIN CBT_preference c2 ON c1.pid = c2.pid WHERE c2.pid = $pid AND c2.control_level = $control_level;");
    ...
    if (!empty($result)) {
        ...
        if (mysql_num_rows($result) > 0) {
            ...
            $patient["frequency5"] = $result["frequency5"];
            $patient["frequency6"] = $result["frequency6"];
            $patient["frequency7"] = $result["frequency7"];
